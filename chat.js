...
const IncidentChat = (function () {
  ...
  function buildSystemPrompt() {
    if (!incident) {
      return 'You are a medical triage support system for an emergency crash detection system called GigGuard. No incident is currently loaded.';
    }
    const i = incident;
    const injuries = (i.ai && i.ai.likely_injuries) ? i.ai.likely_injuries.join(', ') : 'Unknown';

    if (mode === 'driver') {
      return `You are a reassuring support system talking directly to the gig rider who just had a crash.
...
Provide calm, brief, reassuring guidance. The rider may be injured, confused, or scared. Do NOT give complex medical advice, just simple reassurance and basic safe practices while waiting for help. Keep responses extremely brief (1-2 sentences max).`;
    ...
    return `You are a medical triage system for an emergency crash detection system called GigGuard. You help dispatch operators understand crash incidents and make dispatch decisions.
...
- Assessment Severity: ${i.severity}
- Likely injuries: ${injuries}
