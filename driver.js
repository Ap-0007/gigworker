...
(function () {
  ...
  function triggerCrash(prevSpeed) {
    ...
    if (recognition) {
      recognition.onresult = (event) => {
        ...
        if (transcript.includes("cancel") || transcript.includes("stop") || transcript.includes("okay") || transcript.includes("ok")) {
          cancel();
        }
      };
    ...
    $('ovSend').onclick = () => { clearInterval(countdownTimer); if (recognition) { try { recognition.stop(); } catch(e) {} } fireSOS(impact, ai, bb); };
  }
  ...
  async function fireSOS(impact, ai, bb, source) {
    ...
    // Send SMS alerts via Twilio backend
    if (incident.support && incident.support.contacts) {
      incident.support.contacts.forEach(c => {
        if (c.status === 'queued' && c.phone && c.phone !== '108' && c.phone !== '100' && c.phone !== '112') {
          fetch('/api/sms', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              to: c.phone,
              message: `EMERGENCY SOS: ${incident.driver.name} has been in a crash. Location: https://maps.google.com/?q=${incident.location.lat},${incident.location.lng}`
            })
          }).catch(err => console.warn('SMS failed to send', err));
        }
      });
    }
      ...
      try {
        ...
        if (inc) {
          ...
          aiSourceText.innerHTML = aiResult._source !== 'rule-based'
            ? `<span style="color:var(--accent);font-weight:600;">${AI.displayName() || 'System'}</span> — assessment complete`
            : 'Rule-based assessment (System unavailable)';
        ...
        if (updatedInc) {
          ...
          if (firstAid) {
            ...
            updatedInc.log.push({ t: Date.now(), msg: 'First-aid instructions generated by the system' });
            Store.upsert(updatedInc);
            renderSOS(updatedInc);
          }
        }
      } catch (e) {
        console.warn('System processing failed:', e);
        aiSourceIcon.innerHTML = icon('activity',{size:16});
        aiSourceText.textContent = 'Rule-based assessment (System error)';
        aiLoadingShimmer.style.display = 'none';
      }
    } else {
      aiSourceIcon.innerHTML = icon('activity',{size:16});
