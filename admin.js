...
(function () {
  ...
  async function moveUnit(unit, dest, onArrive, syncIncidentId) {
    if (unitTimers[unit.id]) clearInterval(unitTimers[unit.id]);
    const origin = { lat: unit.lat, lng: unit.lng };
    const points = await Geo.fetchOSRMRoute(origin, dest);
    ...
    unitTimers[unit.id] = setInterval(() => {
      ...
      const p = Geo.interpolatePolyline(points, t);
      if (p) { unit.lat = p.lat; unit.lng = p.lng; }
      if (syncIncidentId && p) {
  ...
  function renderTranscript(i) {
    ...
    const lines = [
      ...
      { t: '00:08', msg: `Medical triage packet generated: ${i.severity}, priority ${(i.ai.priority_score || 0).toFixed(1)}/10.` },
  ...
  function renderDetail(i) {
    ...
    $('detail').innerHTML = `
      ...
      <div class="card" style="margin-top:16px;background:var(--bg-2);">
        <div class="card-title" style="display:flex;align-items:center;gap:6px;">${icon('cpu',{size:14})} Medical assessment</div>
  ...
  if (typeof IncidentChat !== 'undefined') {
    ...
    IncidentChat.clear();
    const ct = $('chatTitle');
    if (ct) ct.innerHTML = icon('message', { size: 14 }) + ' ' + (AI.displayName() || 'Support') + ' incident chat';
  }
