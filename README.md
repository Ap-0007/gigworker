...
🚀 **Live Demo:** [https://gigworker-six.vercel.app](https://gigworker-six.vercel.app)

A safety net for gig & delivery riders that fuses three prototypes:

- **BlackBox SOS** — passive crash detection from phone motion sensors + automated medical triage
...
4. The incident pops up **instantly** in the Dispatch Center with an automated medical assessment.
...
| `classify()` medical triage report (severity / injuries / dispatch / priority) | BlackBox SOS `api/classify.js`, `AIClassifier.ts` — same JSON shape |
...
contact mesh status, nearby services, and a persistent support chat panel.
...
## Automated Engine & the API key

Triage, narratives, first-aid, and operator chat are powered by **an automated medical engine**
...
| Any host | a key the user pastes in Settings (localStorage) | direct to engine |

`call()` picks the transport automatically: if a direct key is present it calls the engine;
otherwise it POSTs to the proxy (`api/groq.js`), which injects the key server-side and
forward the request. The proxy returns the exact response shape, so every
higher-level helper is unchanged.

`classify()` stays rule-based and returns the **exact** JSON shape the engine
produces (`severity`, `likely_injuries`, `dispatch`, `hospital_dept`,
`estimated_casualties`, `priority_score`) — it's the automatic fallback whenever the engine is
unavailable.
...
vercel --prod                # deploy; system now works with no key in the client
```
