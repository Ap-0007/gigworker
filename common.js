...
/* ===================================================================
   Store — single source of truth for incidents, shared across tabs/devices.
   Uses Firebase Firestore if configured, otherwise falls back to localStorage.
   =================================================================== */
const Store = (function () {
  ...
  const notify = (reason) => subs.forEach((fn) => { try { fn(localData, reason); } catch (e) { console.error(e); } });

  if (db) {
    db.collection('incidents').orderBy('createdAt', 'desc').onSnapshot((snap) => {
      localData = snap.docs.map(doc => doc.data());
      notify('firestore');
    });
  } else {
    localData = readLocal();
    if (ch) ch.onmessage = (e) => { localData = readLocal(); notify((e.data && e.data.reason) || 'remote'); };
    window.addEventListener('storage', (e) => { if (e.key === KEY) { localData = readLocal(); notify('storage'); } });
  }

  return {
    getAll: () => localData,
    get: (id) => localData.find((i) => i.id === id),
    upsert: (inc) => {
      const idx = localData.findIndex((i) => i.id === inc.id);
      if (idx >= 0) localData[idx] = inc; else localData.unshift(inc);
      
      if (db) {
        db.collection('incidents').doc(inc.id).set(inc);
      } else {
    ...
    clear: () => {
      if (db) {
        localData.forEach(inc => db.collection('incidents').doc(inc.id).delete());
      } else {
        writeLocal([]);
...
const Geo = {
  ...
  async fetchOSRMRoute(start, end) {
    ...
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        // geojson is an array of [lng, lat]
  ...
  // Interpolate along a polyline (array of {lat, lng})
  interpolatePolyline(points, t) {
    ...
    for (let i = 1; i < points.length; i++) {
      const d = Geo.distanceKm(points[i-1], points[i]);
      totalDist += d;
    ...
    for (let i = 1; i < points.length; i++) {
      if (dists[i] >= targetDist) {
        // We are between points i-1 and i
        const segmentDist = dists[i] - dists[i-1];
        if (segmentDist === 0) return points[i];
        const localT = (targetDist - dists[i-1]) / segmentDist;
        return Geo.lerp(points[i-1], points[i], localT);
      }
    }
...
const AI = (function () {
  ...
  function displayName() { const n = name(); return n === 'groq' ? 'System' : null; }
  return {
...
- Medical Assessment: ${incident.ai.likely_injuries.join(', ')}
